...
def validate_filename(filename: str) -> RoutingMetadata:
    ...
    if len(parts) != 5:
        raise FilenameValidationError("Filename must have exactly 5 parts account_audience_name_platform_YYYYMMDD_batchID")
 
    account, eventname, platform, date_str, batchID = parts
 
    if account not in settings.approved_accounts:
        raise FilenameValidationError(f"Unknown account: {account}")
    if not eventname or not eventname.strip():
        raise FilenameValidationError("Audience name cannot be empty")
    if platform != "meta" and platform != "googleads":
        raise FilenameValidationError(f"Invalid platform. Got: {platform}")
    if not re.match(r"^[0-9]{8}$", date_str):
        raise FilenameValidationError(f"Date must be YYYYMMDD format. Got: {date_str}")
    if not batchID.isdigit():
        raise FilenameValidationError(f"batchID must be numeric. Got: {batchID}")

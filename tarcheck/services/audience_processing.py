...
from services.approved_accounts import get_destination
...
def write_audit_log(request_id, routing_metadata, total_rows, valid_rows_count, invalid_rows, dispatch_results, overall_status):
    ...
    record = {
        ...
        "audience_name": routing_metadata.audience_name,
...
async def process_meta_upload(request_id: str, csv_content: str, routing_metadata: RoutingMetadata):
    ...
    db = SessionLocal()
    destination = get_destination(db, routing_metadata.account, routing_metadata.audience_name, "meta")
    db.close()
    if not destination:
        checkpoint(request_id, Checkpoint.PAYLOAD_CREATED, {"error": "no destination configured"})
        write_audit_log(request_id, routing_metadata, len(valid_rows) + len(invalid_rows), len(valid_rows), invalid_rows, [], "failed")
        return

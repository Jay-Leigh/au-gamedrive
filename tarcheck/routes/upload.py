...
async def upload_audience(
    ...
    # Register + hand off
    if not register_batch(routing_metadata.account, routing_metadata.batch_id, request_id):
        raise DuplicateBatchError(f"Duplicate batchID '{routing_metadata.batch_id}' for account '{routing_metadata.account}'. Race with concurrent upload.")

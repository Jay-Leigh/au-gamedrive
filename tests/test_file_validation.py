...
def test_valid_meta_filename():
    result = validate_filename("realbeds_QualifiedLeads_meta_20260526_001_update.csv")
    assert result.account == "realbeds"
    assert result.audience_name == "QualifiedLeads"
...
def test_valid_google_filename():
    result = validate_filename("realbeds_QualifiedLeads_googleads_20260526_001_update.csv")
    assert result.platform == "googleads"

def test_valid_replace_filename():
    result = validate_filename("realbeds_QualifiedLeads_meta_20260526_001_replace.csv")
...
def test_unknown_account_raises():
    with pytest.raises(FilenameValidationError, match="Unknown account"):
        validate_filename("fakeclient_QualifiedLeads_meta_20260526_001_update.csv")
...
def test_invalid_platform_raises():
    with pytest.raises(FilenameValidationError, match="Invalid platform"):
        validate_filename("realbeds_QualifiedLeads_tiktok_20260526_001_update.csv")

def test_bad_date_format_raises():
    with pytest.raises(FilenameValidationError, match="YYYYMMDD"):
        validate_filename("realbeds_QualifiedLeads_meta_2026-05-26_001_update.csv")

def test_non_numeric_batch_id_raises():
    with pytest.raises(FilenameValidationError, match="batchID must be numeric"):
        validate_filename("realbeds_QualifiedLeads_meta_20260526_abc_update.csv")

def test_seven_parts_raises():
    with pytest.raises(FilenameValidationError, match="exactly 6 parts"):
        validate_filename("realbeds_QualifiedLeads_extra_meta_20260526_001_update.csv")

def test_invalid_action_raises():
    with pytest.raises(FilenameValidationError, match="Input should be"):
        validate_filename("realbeds_QualifiedLeads_meta_20260526_001_delete.csv")

def test_all_approved_accounts_pass():
    for account in ["realbeds"]:
        result = validate_filename(f"{account}_QualifiedLeads_meta_20260526_001_update.csv")
